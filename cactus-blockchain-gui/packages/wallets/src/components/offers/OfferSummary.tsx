...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries.length > 0 ? (['cac', 'tcac'].includes(makerEntries[0][0].toLowerCase()) ? mojoToCactus(makerEntries[0][1]) : mojoToCAT(makerEntries[0][1])) : undefined;
  const takerAmount = takerEntries.length > 0 ? (['cac', 'tcac'].includes(takerEntries[0][0].toLowerCase()) ? mojoToCactus(takerEntries[0][1]) : mojoToCAT(takerEntries[0][1])) : undefined;
