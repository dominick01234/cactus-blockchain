...
from cactus.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from cactus.cmds.peer_funcs import print_connections
...
from cactus.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
...
from cactus.wallet.util.address_type import AddressType, ensure_valid_address
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if typ == WalletType.STANDARD_WALLET:
        ...
        final_min_coin_amount = uint64(int(min_coin_amount * units["cactus"]))
    ...
    print(f"To get status, use command: cactus wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
