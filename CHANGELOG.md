...
## 1.5.1 Cactus blockchain 2022-8-23

### Added

- Add Source and Changelog to project_urls (Thanks @strayer!)
- Add condition code constant for REMARK, an always true Cactuslisp condition
- Add several wallet optimizations
- Add `cactus db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the cactuslisp code when necessary
...
- Removing `logging.basicConfig()` from `CactusServer.__init__()`
...
- Log `cactus_full_version_str` in daemon and all services
...
- Substantially reduced overall size of Cactus packages
...
- Make sure cactus commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 Cactus blockchain 2022-7-26
...
- Update `cactus wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the Cactus blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the Cactus blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 Cactus blockchain 2022-6-29
...
- New coin selection algorithm based on bitcoin knapsack. Previously cactus selected the largest coin
- Updated cactuspos to 1.0.10
- Updated cactusvdf to 1.0.6
...
- Updated warning about `CACTUS_ROOT` being set when running init
...
## 1.3.5 Cactus blockchain 2022-5-11
...
## 1.3.4 Cactus blockchain 2022-4-19
...
- Implemented `cactus rpc` command
...
- Added `-n`/`--new-address` option to `cactus wallet get_address`
...
- Fixed typo in command line argument parsing for `cactus db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `cactus configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `cactus db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `cactus show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `cactus wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `cactus wallet show`
...
- You cannot install and run cactus blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Cactus blockchain 2022-4-02
...
- Fixed exception when `cactus keys migrate` is run without needing migration.
...
## 1.3.0 Cactus blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Cactus (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Cactus will not use when launching process pools. Using 0 will allow Cactus to use all cores for process pools. Set the default to 0 to allow Cactus to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, cactus keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using cactus keys derive search.
...
- VDF, cactuspos, and blspy workflows updated to support python 3.10 wheels.
...
- CLI option cactus show will display the currently selected network (mainnet or testnet).
- CLI option cactus plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Thanks to @cross for an optimization to cactuspos to use rename instead of copy if the tmp2 and final files are on the same filesystem.
- Updated to use cactuspos 1.0.9.
...
- Fixed issues with displaying IPv6 address in CLI with cactus show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the cactus-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI cactus show -c when No Info should be displayed.
- Thanks to @madMAx43v3r for fixes in cactuspos related to a possible race condition when multiple threads call Verifier::ValidateProof.
...
- When you close the Cactus app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Cactus and relaunch the Cactus app.
...
## 1.2.11 Cactus blockchain 2021-11-4

### Added

- Farmers rejoice: today's release integrates two plotters in broad use in the Cactus community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/Cactus-Network/cactus-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/Cactus-Network/cactus-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Cactus Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `cactus init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `cactus plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 Cactus blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki/Passphrase-Protected-Cactus-Keys-and-Key-Storage-Migration). We also launched a new Cactuslisp compiler in clvm_tools_rs which substantially improves compile time for Cactuslisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki/Passphrase-Protected-Cactus-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Cactuslisp compiler in clvm_tools_rs in cactus-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other cactus services.
...
- Updated the old comment on cactus/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Cactus DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Cactus blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Cactus blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Cactus's broader set of DID capabilities.
...
- Several clvm_rs updates to support our upcoming Cactus Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install cactus-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the cactus install and then converts it into proper npm format and puts that into package.json.
- Added some new class methods to the Program objects to improve ease of use.
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing Cactuslisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
## 1.2.3 Cactus blockchain 2021-07-26
...
- Updated cactuspos to 1.0.4.
- Included all Cactuslisp files in source distribution.
...
- Thanks @mishan for your change to 'cactus plotnft show' to display Percent Successful Points.
...
## 1.2.2 Cactus blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Cactus-Network/cactus-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Cactus blockchain 2021-07-12
...
## 1.2.0 Cactus blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `cactus plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Cactus-Network/cactus-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Cactus-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `cactus farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `cactus configure` now supports command line updates to peer count and target peer count.
...
- All Cactus repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `cactus plotnft` to manage pools.
...
- We updated cactuspos to version 1.0.3. This adds parallel reads to GetFullProof. Thanks to @marcoabreu ! We now print target/final directory early in the logs refs and log process ID. Thanks to @grayfallstown ! We are now using Gulrak 1.5.6.
@683280 optimized code in phase1.hpp. @jespino and @mrhacky started migrating to flags instead of booleans parameters for `show_progress` and `nobitfield`. If you are providing third-party tools you may need to make adjustments if relying on the cactuspos log.
- Updated cactusvdf to version 1.0.2 to fix certain tests.
...
- We have increased the strictness of validating Cactuslisp in the mempool and clvm.
...
- Correctly display private key in `cactus keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `cactus keys show`.
...
- Thank you to @CactusMineJP for various improvements.
...
- `cactus farm summary` better handles wallet errors.
...
### 1.1.7 Cactus Blockchain 2021-06-05
...
## 1.1.6 Cactus Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /cactus/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `cactus configure -t true|false`).
...
- We now use version 1.0.2 of cactuspos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent cactuspos prover crashing for more bad plots.
...
## 1.1.5 Cactus Blockchain 2021-05-09
...
## 1.1.4 Cactus Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `cactus keys show`. Use `cactus keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Cactus Blockchain 2021-05-01
...
- We changed cactuslisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Cactuslisp programs now remained serialized in Node for better performance.
...
## 1.1.2 Cactus Blockchain 2021-04-24
...
## 1.1.1 Cactus Blockchain 2021-04-21
...
## 1.1.0 Cactus Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Cactus Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of Cactuslisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [cactuslisp.com](https://cactuslisp.com) shortly.
...
## 1.0.5 Cactus Blockchain 2021-04-14
...
## 1.0.4 Cactus Blockchain 2021-04-12
...
- The `src` directory in cactus-blockchain has been changed to `cactus` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `cactus/`
...
## 1.0.3 Cactus Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Cactus-Network/cactus-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `cactus keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of cactuspos 1.0.0 as well.

## 1.0.2 Cactus Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [cactuspos](https://github.com/Cactus-Network/cactuspos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/cactus-blockchain).
...
- @jespino changed `cactus keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `cactus keys show`.
...
## 1.0.1 Cactus Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `cactus keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/cactus-blockchain/).
...
- Users can now pip install e.g. cactus-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Cactus Blockchain 2021-03-17
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/cactus-blockchain).
...
- Migration with CACTUS_ROOT set does not crash cactus init
...
- `cactus farm summary` displayed some incorrect amounts.
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Cactus-Network/cactus-blockchain/issues/1189)
...
- Remove `cactus plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `cactus keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of cactusvdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from cactusvdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- Generators have been re-worked in CLVM. We added a cactuslisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during CactusLisp native deserialization.
...
- `cactus wallet` now takes get_address to get a new wallet receive address from the CLI.
- `cactus plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- Cactuslisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of cactus-blockchain, etc., instead of in blspy.
...
- In cactusvdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new cactusvdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in cactusvdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for cactusvdf.
- cactusbip158 was set to a gold 1.0 version.
- Comments to Cactuslisp and clvm source have been updated for all of the Cactuslisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in cactus-blockchain.
- aiohttp was updated to 3.7.4 to address a low severity [security issue](https://github.com/advisories/GHSA-v6wp-4m6f-gcjg).
- calccrypto/uint128_t was updated in the Windows cactuspos implementation. Cactuspos required some changes its build process to support MacOS ARM64.
...
- Various typos and style clean ups were made to the Click CLI implementation. `cactus -upnp f` was added to disable uPnP.
- `cactus plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/cactus-blockchain).
- You can obtain a new wallet receive address on the command line with `cactus wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All cactuslisp opcodes have been renumbered. This should be the last major breaking change for cactuslisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Cactus-Network/cactus-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `cactus -h` and `cactus SUBCOMMAND -h` can be your guide.
...
- `cactus plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- `cactus configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Cactus-Network/cactus-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and cactuslisp are in release form. We have one major change to cactuslisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern CactusSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'cactus wallet get_transactions' in the command line to see your transactions.
- 'cactus wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Cactus Blockchain GUI](https://crowdin.com/project/cactus-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Cactus-Network/cactus-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `cactus plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- We updated cactuspos to hopefully address some harvester crashes when moving plot files.
...
- You can now send and receive transactions with the command line. Try `cactus wallet -h` to learn more. Also, `cactus wallet` now requires a third argument of `show`, therefor you will use `cactus wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [cactus blockchain gui](https://crowdin.com/project/cactus-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `cactus init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `cactus init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
- We build both MacOS x86_64 and MacOS universal wheels for cactuspos, cactusvdf, blpsy, and cactusbip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- `cactus plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Cactus-Network/cactus-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Cactuslisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Cactus-Network/cactus-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, cactuspos, cactusvdf, and cactusbip158.
...
- `cactus plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- `cactus plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `cactus show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `cactus show` handles sub block 0 better.
...
- `cactus plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `cactus plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `cactus show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `cactus configure --enable-upnp`.
...
- `cactus configure` now lets you enable or disable uPnP.
...
- You can now specify which private key to use for `cactus plots create`. After obtaining the fingerprint from `cactus keys show`, try `cactus plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for cactusvdf from the current release of gmp-6.2.1 which we have upgraded cactusvdf and blspy to support.
- There is a new cli command - `cactus configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `cactus configure -h`. Note that if cactus services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- All appropriate Cactuslisp smart transactions have been updated to use aggsig_me.
...
- cactusvdf now allows passing in input to a VDF for new consensus.
...
- Changes were made to cactuspos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run cactus-blockchain.
...
- `cactus show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Cactus-Network/cactuspos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Cactus dependencies like cactusbip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Cactus-Network/cactus-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install cactus-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Cactus-Network/cactus-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Cactus-Network/cactus-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Cactus GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Cactus-Network/cactus-blockchain/tree/dev/electron-react/src/locales) of the cactus-blockchain repository.
- `cactus check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to cactuspos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [cactuspos release notes](https://github.com/Cactus-Network/cactuspos/releases/tag/0.12.32).
...
- cactuspos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- cactusvdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in cactuspos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all cactus services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test cactus since Beta 12 should appear in your wallet.
...
- Cactuspos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in cactuspos
...
- Long_description_content_type is now set to improve cactus-blockcactusn's Pypi entry. Thanks to @altendky for this pull request.
...
- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test cactus are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.cactus` directory.
...
- `cactus init` now automatically discovers previous releases in each new release.

### Fixed

- `cactus show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `cactus show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Cactus Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Cactus UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (cactus run farmer-only, wallet-only, timelord-only)
...
- `cactus show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `cactus show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Cactus simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Cactus implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test cactus between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test cactus from the earlier two releases however.
...
- The proof of space document in cactuspos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `cactus show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Cactus)
- Various code review alerts for comparing to a wider type in cactuspos were fixed. Additionally, unused code was removed from cactuspos
...
- See wallet balances in command line: `cactus show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `cactus keys sign` and `cactus keys verify` to allow farmers to certify their ownership of keys.
...
- `cactus start wallet-server` changed to `cactus start wallet`, for consistency.
...
- Removed legacy scripts such as cactus-stop-server, cactus-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Cactus-Network/bls-signatures) to use the
...
- The various sub repositories (cactuspos, cactusvdf, etc.) now build ARM64 binary
...
- The cactus executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./cactus -h` from
`~\AppData\Local\Cactus-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Cactus.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"cactus plots create" and "cactus plots check".
- Add plot directories to config.yaml from the cli with "cactus plots add".
...
- Linux builds of cactusvdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Cactus Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.cactus/test-plots/`
- We have completely refactored all networking code towards making each Cactus service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make cactus start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Cactus Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting cactus now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'cactus keys' command line.
...
- We added a `cactus keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the cactus-blockchain, cactuspos, cactusvdf, cactusbip158, and bls-library repositories to add some automated security analysis to our ci.
...
- cactuspos is now easier to compile with MSVC.
...
- `cactus-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `cactus-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- Building the sub reposities (cactuspos, cactusvdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Cactus Proof of Space ([cactuspos](https://github.com/Cactus-Network/cactuspos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `cactus init` then sync on first start will happen automatically. "\$CACTUS_ROOT" users will need to delete `$CACTUS_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki) and placed in [INSTALL](https://github.com/Cactus-Network/cactus-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Cactus-Network/cactus-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of cactus action commands. You can run `cactus version` or `cactus start node` for example. Just running `cactus` will show you more options. However `cactus-create-plots` continues to use the hyphenated form. Also it's now `cactus generate keys` as another example.
- Cactus start commands like `cactus start farmer` and `cactus stop node` now keep track of process IDs in a run/ directory in your configuration directory. `cactus stop` is unlikely to work on Windows native for now. If `cactus start -r node` doesn't work you can force the run/ directory to be reset with `cactus start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Cactus-Network/cactus-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `cactus start wallet-gui` has been chaned to `cactus start wallet` which launches but the GUI and server on MacOS and Linux. `cactus start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Cactus-Network/cactus-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `cactus stop all` command sometimes fails, use `cactus-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `cactus init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `cactus-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Cactus-Network/cactus-blockchain/wiki/Updating-beta-software). If you've set `$CACTUS_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `cactus show`.

### Changed

- `cactus-restart-harvester` has been renamed from `cactus-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.cactus/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Cactus blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Cactus-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the cactus management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `cactus-start-farmer &`. The [README.md](https://github.com/Cactus-Network/cactus-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.cactus/release/config, plots/ db/, wallet/ etc. This can be overridden by `export CACTUS_ROOT=~/.cactus` for example which would then put the plots directory in `HomeDirectory/.cactus/plots`.
- The libraries cactus-pos, cactus-fast-vdf, and cactus-bip-158 have been moved to their own repositories: [cactuspos](https://github.com/Cactus-Network/cactuspos), [cactusvdf](https://github.com/Cactus-Network/cactusvdf), and [chaibip158](https://github.com/Cactus-Network/cactusbip158). They are brought in by cactus-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Cactus-Network/bls-signatures).
- The install process now brings in cactuspos, cactusvdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
- `cactus-regenerate-keys` has been renamed `cactus-generate-keys`.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for cactusvdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/Cactus-Network/cactus-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `cactus show -s -c` and try `cactus show --help` for usage instructions.
...
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/cactusvdf/fast_vdf/.
...
- Cactus fullnode, farmer and harvester now default to logging to cactus.log in the cactus-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to cactus_full node, cactus_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new cactus services were started.
...
[1.0beta4]: https://github.com/Cactus-Network/cactus-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Cactus-Network/cactus-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Cactus-Network/cactus-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Cactus-Network/cactus-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Cactus-Network/cactus-blockchain/releases/tag/Alpha-1.0
