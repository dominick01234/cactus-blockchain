...
from cactus.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from cactus.plotting.create_plots import create_plots, PlotKeys
...
from cactus.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from cactus.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
...
from cactus.util.config import load_config, lock_config, save_config, override_config
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="cactus-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `cactus generate keys`")
