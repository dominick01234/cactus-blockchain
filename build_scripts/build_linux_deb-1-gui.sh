...
echo "Remove unused @cactus packages to make cache slim"
ls -l packages
rm -rf packages/api
...
# Remove `packages/gui/node_modules/@cactus` because it causes an error on later `electron-packager` command
rm -rf "@cactus"
